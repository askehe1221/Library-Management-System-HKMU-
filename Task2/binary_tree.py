...
class Tree():
    ...
    def add(self, item):
        """层序添加节点 / Add node by level order"""
        node = Node(item)
    ...
    def find_min(self, node):
        """查找最小值节点 / Find minimum value node"""
        if node is None:
            return None
    ...
    def find_max(self, node):
        """查找最大值节点 / Find maximum value node"""
        if node is None:
            return None
    ...
    def is_empty(self):
        """检查树是否为空 / Check if tree is empty"""
...
# 查找最大最小值 / Find min and max values
